...
import { SEMRUSH_DATABASES, SEMRUSH_URL } from "../constants";
import { useInfiniteKeywordSearchQuery } from "../api";
import {
  Columns,
  KeywordSearchSort,
  KeywordSearchType as SearchType,
} from "../semrush";
...
export function KeywordsSearch() {
  ...
  const [keywordArgs, setKeywordArgs] = useState<KeywordSearchArgs>({
  ...
  const {
    ...
    ...keywordArgs,
    // Prevent end whitespace causing refetch
    keyword: debouncedKeyword.trimEnd(),
    limit: 16,
  });
  ...
  const handleSort = useCallback(
    (newColumn: Columns) => {
      const currentSort = keywordArgs.sort;
      ...
      <div className="flex items-center justify-center h-[545px] relative">
        ...
        {!isLoading && (data === undefined || keyword === "") && (
          <p className="text-tertiary text-center max-w-[300px]">
            Find new ranking opportunities in just a few clicks with a database
            of 25 billion keywords.
          </p>
        )}
        {data?.length === 0 && keyword && (
          <p className="text-tertiary">Nothing returned.</p>
        )}
        {data && data.length > 0 && keyword && (
          <TableContainer className="self-start">
            <TableHead className="pr-4">
              {columnHeadings.map(({ sortColumn, className, name }, i) => {
                const { column, order } = keywordArgs.sort;
            ...
            <TableBody>
              {data.map((row, i) => (
                <KeywordRow key={i} row={row} database={keywordArgs.database} />
              ))}
...
type KeywordRow = NonNullable<
  ReturnType<typeof useInfiniteKeywordSearchQuery>["data"]
>[0];
  ...
  return (
    <TableRow className="min-h-[40px]">
      <TableCell className="grow">
        <a
          target="_blank"
          href={`${SEMRUSH_URL}/analytics/keywordoverview/?q=${keyword}&db=${database}`}
